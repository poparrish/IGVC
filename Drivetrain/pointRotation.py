...
def calcJoyBearing(xAxis,yAxis):#joystick only.this try is just sorting out the right bearings and quadrants from the joystick inputs...we will not use this for the auto-nav
    try:
        ...
        if yAxis < 0:  # sort out the bearing and quadrants
            if xAxis < 0:
                ...
                quadrant = 3
		bearing = bearing *-1 #quadrant 3 is negative
            else:
                bearing = 180 - bearing
                if xAxis == 0:
                    quadrant = 0
            ...
            if xAxis < 0:
                quadrant = 2
		bearing = bearing *-1 #quadrant 2 is negative
            else:
                if xAxis == 0:
                    quadrant = 0
                else:
